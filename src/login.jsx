...
const LoginPage = () => {
    ...
    const processCaptcha = async () => {
        ...
        const tokenCodeResult = await tokenCodeRequest.json();
            ...
            <Row className="justify-content-md-center width-100 height-100">
                <Col xs={12} sm={8} md={6} lg={4} className="login-form-container shadow margin-auto">
                    ...
                    <Form onSubmit={loginEvent}>
                        <Alert variant="danger" hidden={errorValue ? false : true}>{errorValue}</Alert>
                        <Alert variant="success" hidden={successValue ? false : true}>{successValue}</Alert>

                        <InputGroup>
                            <InputGroup.Text>手机号</InputGroup.Text>
                            <Form.Control type="number" name="symbol" title="手机号" placeholder="13333333333" disabled={passportReadOnly}
                                value={symbolValue} onChange={event => setSymbolValue(event.target.value)} />
                        </InputGroup>

                        <InputGroup>
                            <InputGroup.Text>密码</InputGroup.Text>
                            <Form.Control type="password" name="password" title="密码" placeholder="123456" disabled={passportReadOnly}
                                value={passwordValue} onChange={event => setPasswordValue(event.target.value)} />
                        </InputGroup>

                        <Form.Check type="checkbox" name="我已阅读并同意各种条款" title="我已阅读并同意各种条款" disabled={passportReadOnly}
                            label={<div>
                                我已阅读并同意
                                <a href="https://app.xueersi.com/xueersi-mall-hm-xbjtoxes/agreement" target="_blank">《学而思网校用户协议》</a>、
                                <a href="https://app.xueersi.com/xueersi-mall-hm-xbjtoxes/privacy" target="_blank">《用户个人信息保护政策》</a>、
                                <a href="https://app.xueersi.com/xueersi-mall-hm-xbjtoxes/childpolicy" target="_blank">《儿童个人信息保护规则》                                </a>、
                                <a href="/eula.html" target="_blank">《NewXesFrontend 最终用户协议》</a>、
                            </div>}
                            value={privacyValue} onChange={event => setPrivacyValue(event.target.value)} />

                        <InputGroup hidden={visuallyValue}>
                            <InputGroup.Text>验证码</InputGroup.Text>
                            <InputGroup.Text><img src={captchaBase64} alt="验证码" /></InputGroup.Text>
                            <Form.Control type="text" name="captcha" title="验证码" placeholder="abcd"
                                value={captchaAnswer} onChange={event => setCaptchaAnswer(event.target.value)} />
                        </InputGroup>
