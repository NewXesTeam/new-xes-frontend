...
const processCaptcha = async () => {
    const tokenCodeRequest = await fetch('https://passport.100tal.com/v1/web/login/pwd', {
        method: 'POST',
        headers: {
    ...
    if (tokenCodeResult.errcode !== 0) {
        ...
        setTimeout(() => {
            location.href = 'https://code.xueersi.com/';
        }, 500);
    }
};

const getCaptcha = async () => {
    const captchaRequest = await fetch('https://passport.100tal.com/v1/web/captcha/get', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
            'client-id': '111101',
                ...
                <v-form class="flex flex-col gap-2" @submit.prevent="onSubmit" ref="form">
                    ...
                    >
                        <template v-slot:append>
                            ...
                            <img :src="captchaBase64" class="h-10" alt="验证码" />
                        </template>
                    </v-text-field>
