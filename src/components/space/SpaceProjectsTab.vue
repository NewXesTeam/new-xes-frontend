...
const [spaceWorksData, loadSpaceWorksData] = useFetchData<SpaceWorks>(
    () => `/api/space/works?user_id=${userId}&page=${currentPage.value}&per_page=20&order_type=${orderType.value}`,
);
const totalPages = computed(() => Math.max(Math.ceil((spaceWorksData.value.data?.total ?? 0) / 20), 1));
